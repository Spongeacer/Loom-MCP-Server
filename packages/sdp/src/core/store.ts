...
import type { Entry, Binding, WorkingSet, SdpConfig, ArtifactEntry } from '../types/index.js';
...
export function initWorkspace(projectName: string, cwd?: string): void {
  ...
  fs.writeFileSync(paths.activePrompt, '<sdp_context>\n  <protocol>SDP initialized. No active task yet.</protocol>\n</sdp_context>');
}
...
export function listEntries(cwd?: string): Entry[] {
  ...
  for (const dir of dirs) {
    ...
    for (const file of fs.readdirSync(dir)) {
      if (file.endsWith('.sdpe.yml') || file.endsWith('.yml')) {
...
export function saveEntry(entry: Entry, cwd?: string): void {
  ...
  const filePath = path.join(dir, `${entry.id}.sdpe.yml`);
...
export function getConfig(cwd?: string): SdpConfig | null {
  const paths = getPaths(cwd);
  if (!fs.existsSync(paths.config)) return null;
  return YAML.parse(fs.readFileSync(paths.config, 'utf-8')) as SdpConfig;
}
