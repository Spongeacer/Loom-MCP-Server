...
server.setRequestHandler(ListToolsRequestSchema, async () => {
  return {
    tools: [
      {
        name: 'sdp_status',
        description: 'Get the current slot-based SDP context including active task, working set, decisions, and risks. Also updates cache/active-prompt.txt.',
        inputSchema: { type: 'object', properties: {} },
      },
      {
        name: 'sdp_expand',
        description: 'Expand an SDP entry to see its full details (L3).',
        inputSchema: {
          type: 'object',
          properties: {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  switch (name) {
    ...
    case 'sdp_read_prompt': {
      const promptPath = path.join(process.cwd(), '.sdp', 'cache', 'active-prompt.txt');
      if (!fs.existsSync(promptPath)) {
        return { content: [{ type: 'text', text: 'SDP not initialized or active-prompt.txt missing.' }] };
      }
...
async function main() {
  console.error('[SDP MCP] Starting stdio transport...');
  const transport = new StdioServerTransport();
  await server.connect(transport);
  console.error('[SDP MCP] Connected.');
}
