...
export function readWalEvents(projectRoot: string, limit = WAL_READ_LIMIT, filterType?: string): WalEvent[] {
  const paths = getPaths(projectRoot);
  const raw = readTextFile(paths.wal);
  if (!raw) return [];
...
export function readWalEventsSince(projectRoot: string, since: string): WalEvent[] {
  ...
  // Fallback: if tail chunk didn't yield enough, do a bounded full read
...
export function summarizeSession(projectRoot: string, hoursBack = SESSION_DEFAULT_HOURS_BACK): string {
