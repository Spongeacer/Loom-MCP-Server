...
import { HEALTH_STALE_DAYS } from './constants.js';
...
function analyzeArtifactHealth(
  ...
  // 4. Orphan
  const relatedBindings = allBindings.filter((b) => b.source === artifact.id || b.target === artifact.id);
  const referencedByEntries = allEntries.filter(
    (e) =>
      e.id !== artifact.id &&
      (e.activation.paths.includes(artifact.artifact.path) || e.activation.entry_refs.includes(artifact.id))
  );
  ...
  const duplicates = allArtifacts.filter(
    (a) => a.id !== artifact.id && a.artifact.fs.exists && a.artifact.content_hash === hash && hash !== ''
  );
...
export function runHealthAnalysis(
  ...
  const byStatus: Record<ArtifactEntry['artifact']['health']['status'], ArtifactEntry[]> = {
    healthy: [], stale: [], orphan: [], legacy: [], redundant: [], missing: [],
  };
