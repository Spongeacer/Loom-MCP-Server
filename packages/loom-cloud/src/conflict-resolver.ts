...
import type { ConflictResult, ConflictStrategy, EntrySyncState } from './types.js';
...
export function resolveConflict(
  local: Entry | null,
  cloud: Entry | null,
  syncState: EntrySyncState | undefined,
  cloudVersion?: number,
): ConflictResult {
  ...
  if (!local && !cloud) {
    return {
      winner: null as unknown as Entry,
      fork: null,
      strategy: 'cloud-wins',
  ...
  if (resolvedCloudVersion > localCloudVersion) {
    ...
    if (localWasModified) {
      ...
      return {
        ...
        strategy: 'fork-local',
        reason:
          `Cloud merged version ${cloudVersion} is newer than local ` +
  ...
  return {
    ...
    strategy: 'local-wins',
    reason:
      `Local version ${localEntry.version} is ahead of or equal to ` +
 ...
 */
function extractCloudVersion(entry: Entry): number {
  const meta = (entry as unknown as Record<string, unknown>).metadata;
  if (meta && typeof meta === 'object' && 'sync' in meta) {
    const sync = (meta as Record<string, unknown>).sync;
    if (sync && typeof sync === 'object' && 'cloudVersion' in sync) {
      const cv = (sync as Record<string, unknown>).cloudVersion;
 ...
 */
function createFork(original: Entry): Entry {
...
export function isEntryDirty(
  ...
  if (!syncState) return true; // Never synced = dirty
