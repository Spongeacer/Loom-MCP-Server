...
describe('cloud-api', () => {
  it('throws CloudApiError on timeout', async () => {
    ...
    try {
      ...
      assert.ok((err as CloudApiError).message.includes('timed out'));
      assert.strictEqual((err as CloudApiError).status, 408);
    }
  });
  ...
  it('throws CloudApiError on HTTP error', async () => {
    ...
    try {
      ...
      assert.ok(err instanceof CloudApiError);
      assert.strictEqual((err as CloudApiError).status, 500);
    }
  });
  ...
  it('throws NO_TOKEN when token is missing', async () => {
    ...
    try {
      ...
      assert.ok(err instanceof CloudApiError);
      assert.ok((err as CloudApiError).message.includes('No access token'));
      assert.strictEqual((err as CloudApiError).code, 'NO_TOKEN');
    }
  });
  ...
  it('successfully registers a device', async () => {
    const mockFetch = async (_url: string, init?: RequestInit) => {
      ...
      return new Response(
        JSON.stringify({ accessToken: `tok_${body.deviceId}`, expiresAt: Date.now() / 1000 + 3600 }),
